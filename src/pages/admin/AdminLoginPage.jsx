    ...
    <div style={s.page}>
      <div style={s.card}>
        <div style={s.logo}>
          ...
          <span style={s.logoText}>AI<span style={s.accent}>Market</span> <span style={s.adminLabel}>Admin</span></span>
        </div>
        <h1 style={s.title}>管理者ログイン</h1>
        ...
        <form onSubmit={handleSubmit} style={s.form}>
          ...
          <div style={s.field}>
            ...
            <div style={s.passWrap}>
              <input style={{ ...s.input, paddingRight: 44 }} type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required autoComplete="current-password" />
...
const s = {
  ...
  card: { width: '100%', maxWidth: 400, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 20, padding: '40px 36px', boxShadow: '0 0 60px rgba(139,92,246,0.08)' },
  logo: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 28 },
  logoIcon: { width: 36, height: 36, background: 'linear-gradient(135deg, #8b5cf6, #22d3ee)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#fff', fontWeight: 900 },
  ...
  adminLabel: { fontSize: 11, color: '#8b5cf6', background: 'rgba(139,92,246,0.15)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: 4, padding: '1px 6px', marginLeft: 6, fontWeight: 700, verticalAlign: 'middle' },
  title: { fontSize: 20, fontWeight: 700, color: '#e2e8f0', textAlign: 'center', margin: '0 0 24px' },
  ...
  submitBtn: { width: '100%', padding: '13px', borderRadius: 10, fontSize: 15, fontWeight: 700, color: '#fff', background: 'linear-gradient(135deg, #8b5cf6, #6d28d9)', border: 'none', cursor: 'pointer', marginTop: 4 },
}
